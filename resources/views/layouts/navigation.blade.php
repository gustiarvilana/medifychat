...
<aside class="flex flex-col h-full py-xl px-base fixed left-0 top-0 z-40 bg-surface border-r border-outline-variant w-[280px] hidden md:flex">
    <div class="mb-2xl px-md">
        <div class="flex items-center gap-base">
            ...
            <div>
                <h1 class="font-headline-lg text-headline-lg font-bold text-primary">MedBot</h1>
    ...
    <nav class="flex-1 space-y-xs">
        <a href="{{ route('dashboard') }}"
            class="flex items-center gap-md px-md py-sm cursor-pointer transition-all duration-200 rounded-xl {{ request()->routeIs('dashboard') ? 'bg-surface-container text-primary font-semibold' : 'text-on-surface-variant hover:text-primary hover:bg-surface-container-low' }}">
            <span class="material-symbols-outlined">dashboard</span>
            <span class="font-body-sm text-body-sm">Dashboard</span>
        </a>
        <a href="{{ route('settings') }}"
            class="flex items-center gap-md px-md py-sm cursor-pointer transition-all duration-200 rounded-xl {{ request()->routeIs('settings') ? 'bg-surface-container text-primary font-semibold' : 'text-on-surface-variant hover:text-primary hover:bg-surface-container-low' }}">
    ...
    <div class="mt-auto px-md space-y-xs">
        <a href="{{ route('profile.edit') }}"
            class="flex items-center gap-md px-md py-sm cursor-pointer text-on-surface-variant hover:text-primary transition-all duration-200 rounded-xl">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full flex items-center gap-md px-md py-sm cursor-pointer text-on-surface-variant hover:text-primary transition-all duration-200 rounded-xl">
...
<nav class="md:hidden fixed bottom-0 left-0 right-0 bg-surface-container-lowest border-t border-outline-variant z-50 flex justify-around py-sm px-md">
    ...
    <a href="{{ route('settings') }}"
        ...
        <span class="material-symbols-outlined {{ request()->routeIs('settings') ? 'font-bold' : '' }}">settings</span>
